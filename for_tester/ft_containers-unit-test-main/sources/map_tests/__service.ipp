...
void fillMap(std::map<T, V, C> &mp) {
  ...
  mp.insert(std::make_pair(5, 3));
  mp.insert(std::make_pair(6, 3));
  mp.insert(std::make_pair(7, 3));
  mp.insert(std::make_pair(8, 3));
  mp.insert(std::make_pair(9, 3));
}

template <class T, class V, class C>
void fillMap(_map<T, V, C> &mp) {
  mp.insert(_make_pair(1, 3));
  mp.insert(_make_pair(2, 3));
  mp.insert(_make_pair(3, 3));
  mp.insert(_make_pair(4, 3));
  mp.insert(_make_pair(5, 3));
  mp.insert(_make_pair(6, 3));
  mp.insert(_make_pair(7, 3));
  mp.insert(_make_pair(8, 3));
  mp.insert(_make_pair(9, 3));
}
...
int run_map_unit_test(std::string test_name, std::vector<int>(func1)(std::map<T, V>),
                      std::vector<int>(func2)(_map<T, V>)) {
  int                result;
  int                leaks;
  time_t             t1;
  time_t             t2;
  ...
  (t1 >= t2) ? printElement(GREEN + std::to_string(t2) + "ms" + RESET)
             : printElement(REDD + std::to_string(t2) + "ms" + RESET);
  (t1 > t2) ? printElement(REDD + std::to_string(t1) + "ms" + RESET)
            : printElement(GREEN + std::to_string(t1) + "ms" + RESET);
